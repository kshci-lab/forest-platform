...
class ThinkingProcess { // forestMRN: forest Meeting Reflection Network
    ...
    loadOkCoreOrganizationOptions(callback){
        $.ajax({
            url: "../php/ok_core_groups.php",
            type: "GET",
            dataType: "json",
            success: function(res){
                if(!res || res.status !== "ok" || !Array.isArray(res.groups)){
                    callback([]);
                    return;
                }
                callback(res.groups.map(function(group){
                    return {
                        value: String(group.group_id || ""),
                        text: group.name ? String(group.name) : String(group.group_id || "")
                    };
                }).filter(function(group){ return group.value !== ""; }));
            },
            error: function(){
                callback([]);
            }
    ...
    selectShareOrganization (){
        const conmenu = document.getElementById('t_Process_conmenu');
        if(conmenu) conmenu.style.display = "none";

        const selected_node_id = defaultThinkingProcess.ownNetwork.getSelection().nodes[0];
        if(!selected_node_id){
            alert('共有する思考ノードを選択してください。');
            return;
        }

        this.loadOkCoreOrganizationOptions((organizations) => {
            if(!organizations.length){
                alert('OK-Core側に共有可能な組織が見つかりません。OK-Coreの knowledge_groups / kgroup_user_link を確認してください。');
                return;
            }
            ...
            organizations.forEach(function(org, index){
                ...
                organizationList += number + ". " + org.text + "\n";
                organizationMap[number] = org;
            });
            ...
            const key = selectedNumber.trim();
            if(!key || !organizationMap[key]){
                alert("無効な番号です。もう一度やり直してください。");
                return;
            }

            const selectedOrg = organizationMap[key];
            const selNode = defaultThinkingProcess.nodes.get(selected_node_id) || {};
            const selected_contents = (selNode.label || '').toString().replace(/\n+/g, ' ').trim();
            this.shareKnowledgeFragmentToOrganization(selectedOrg.value, selected_node_id, selected_contents);
        });
            ...
            $.ajax({
                ...
                success: function(resp) {
                    ...
                    try {
                        ...
                        console.warn('share fragment response parse failed:', e);
                    }
