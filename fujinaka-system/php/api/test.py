...
messages = [
    {"role": "system", "content": "あなたは三角ロジックの妥当性を精査する評論家です。欠損・曖昧・論理飛躍・用語未定義を厳密に指摘し、改善提案を与えてください。憶測で新情報を創作してはいけません。"},
    {"role": "user", "content": f"三角ロジックを今から以下の形式「事実[]に対して理由付け[]をすることで[]という主張をしている。」で送ります。三角ロジックとは、論証構造を抽出する際の枠組みや論説教材の構成要素の関係を考える際の分析モデルであるToulminモデルから「主張」「事実」「理由付け」の3要素で構成されるモデルである。事実は主張の基礎となる事実です。理由付けはデータがサポートするかについての言明です。1.事実や理由づけの内容が妥当であるか。\n2.論理のつながり（主張、事実、理由づけの関係の適切さ）が適切であるか\n---\n{triangle_text}"}
]
