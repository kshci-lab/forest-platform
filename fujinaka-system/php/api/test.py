...
messages = [
    {
        ...
        "content": (
            ...
            "■ 出力形式\n"
            "-----------------------------------------\n\n"
            "■ 出力形式（重要：JSONで返してください）\n"
            "-----------------------------------------\n\n"
            "以下のJSON配列形式で返してください：\n\n"
            "[\n"
            "  {\n"
            "    \"claim_id\": \"<id>\",\"claim\": \"<主張の内容そのまま>\",\n"
            "    \"advice\": \"<なぜその三角ロジックが妥当でない可能性があるのか、どの点が妥当でない可能性があるのか、具体的な理由や改善案を含めて詳しく記述してください。>\"\n"
            "  },\n"
            "  ...\n"
            "]\n\n"
            "例：\n"
            ...
            "  \"claim_id\": \"1\",\n"
            "  \"claim\": \"AはBである\",\n"
            "  \"advice\": \"根拠として挙げられているCは、Aの主張と直接的な因果関係が示されていないため、主張の妥当性に疑問が残る可能性がある。具体的には、CがAを支持する論拠として十分かどうか、追加の説明やデータが必要です。\"\n"
            "}\n\n"
            "助言が存在しない場合、その三角ロジックは配列に含めない。\n"
            "妥当と判断した場合は一切出力しない。\n"
            "JSON以外の文字列は一切出さないこと."
        ),
    },
