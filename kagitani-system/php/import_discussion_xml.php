...
$pdo = null;
$portsToTry = [8889, 3306, null];
foreach ($portsToTry as $p) {
    try {
        if ($p === null) {
            $dsn = 'mysql:host=localhost;dbname=forest_platform;charset=utf8';
            $pdo = new PDO($dsn, 'root', 'root', [PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION, PDO::ATTR_DEFAULT_FETCH_MODE => PDO::FETCH_ASSOC]);
        } else {
            $dsn = sprintf('mysql:host=localhost;port=%d;dbname=forest_platform;charset=utf8', $p);
            $pdo = new PDO($dsn, 'root', 'root', [PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION, PDO::ATTR_DEFAULT_FETCH_MODE => PDO::FETCH_ASSOC]);
        }
        // 成功したら抜ける
        break;
...
if (!$pdo) {
    try {
        // 最後に例外メッセージを取得して返すため単純に新しい接続を試みてエラーメッセージを得る
        $dsn = 'mysql:host=localhost;dbname=forest_platform;charset=utf8';
        $tmp = new PDO($dsn, 'root', 'root');
    } catch (Exception $e) {
